...
import io.kubesphere.client.util.credentials.AccessTokenAuthentication;
import io.kubesphere.client.util.credentials.Authentication;
import io.kubesphere.client.util.credentials.KubeconfigAuthentication;
...
public class ClientBuilder {
  ...
  private static File findHomeDir() {
    final String envHome = System.getenv(KubeConfig.ENV_HOME);
  ...
  private static File findConfigInHomeDir() {
    ...
    if (homeDir != null) {
      final File config = new File(new File(homeDir, KubeConfig.KUBEDIR), KubeConfig.KUBECONFIG);
      if (config.exists()) {
        return config;
      }
    }
